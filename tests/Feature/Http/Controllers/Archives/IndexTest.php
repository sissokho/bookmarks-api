...
class IndexTest extends TestCase
{
    ...
    {
        $response = $this->getJson(route('api.v1.favorites.index'));

        $response->assertUnauthorized();
    }

    /** @test */
    public function archived_bookmarks_are_listed_by_the_most_recent_in_a_paginated_way(): void
    {
        ...
        $response = $this->getJson(route('api.v1.archives.index'));
        ...
        $response->assertOk()
            ...
            ->assertJson([
                'links' => [
                    'first' => route('api.v1.archives.index', ['page' => 1]),
                    'last' => route('api.v1.archives.index', ['page' => 2]),
                    'prev' => null,
                    'next' => route('api.v1.archives.index', ['page' => 2]),
                ],
                'meta' => [
                    'current_page' => 1,
                    'from' => 1,
    ...
    /** @test */
    public function only_users_archived_bookmarks_are_shown(): void
    {
        $user = User::factory()->create();
    ...
    /** @test */
    public function only_archived_bookmarks_are_returned(): void
    {
        $user = User::factory()->create();
        ...
        Bookmark::factory()  // normal - not archived
            ->for($user)
            ->create();

        Bookmark::factory() // favorite - not archived
            ->for($user)
            ->favorite()
            ->create();
        ...
        $response->assertOk()
            ->assertJson(
                fn (AssertableJson $json) => $json->hasAll(['meta', 'links'])
                    ...
                    ->has(
                        'data.0',
                        fn ($json) => $json->where('id', $favoriteArchivedBookmark->id)
                            ->where('title', $favoriteArchivedBookmark->title)
                            ->where('url', $favoriteArchivedBookmark->url)
                            ->where('favorite', true)
                            ->where('archived', true)
                    ...
                    ->has(
                        'data.1',
                        fn ($json) => $json->where('id', $normalArchivedBookmark->id)
                            ->where('title', $normalArchivedBookmark->title)
                            ->where('url', $normalArchivedBookmark->url)
                            ->where('favorite', false)
                            ->where('archived', true)
                            ->where('created_at', $normalArchivedBookmark->created_at->toDateTimeString())
                            ->where('tags', [])
                    )
            );
    }
    ...
    public function user_can_customize_the_pagination(): void
    {
        ...
        $response = $this->getJson(route('api.v1.archives.index', ['per_page' => 10, 'page' => 2]));
        ...
        $response->assertOk()
            ...
            ->assertJson([
                'links' => [
                    'first' => route('api.v1.archives.index', ['page' => 1]),
                    'last' => route('api.v1.archives.index', ['page' => 2]),
                    'prev' => route('api.v1.archives.index', ['page' => 1]),
                    'next' => null,
                ],
                'meta' => [
                    'current_page' => 2,
    ...
    /** @test */
    public function user_can_search_through_his_archived_bookmarks(): void
    {
        $user = User::factory()->create();
        ...
        $response = $this->getJson(route('api.v1.archives.index', ['search' => 'down']));
     ...
     */
    public function user_can_customize_the_ordering_of_his_archived_bookmarks(Order $order): void
    {
        $user = User::factory()->create();
        ...
        $response = $this->getJson(route('api.v1.archives.index', ['order_by' => $order->value]));
    ...
    public function parameters_are_validated($params, $errors): void
    {
        ...
        $response = $this->getJson(route('api.v1.archives.index', $params));
